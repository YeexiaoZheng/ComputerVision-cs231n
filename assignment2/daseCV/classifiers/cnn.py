...
class ThreeLayerConvNet(object):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
        ...
        # 如提示所述, 通过loss中的处理可以得知最终实际上经过conv是维持了size不改变的
    ...
    def loss(self, X, y=None):
        ...
        # 其次计算gradient
        dout, dw3, db3  = affine_backward(dout, affine2_cache)
        dout, dw2, db2 = affine_relu_backward(dout, affine1_cache)
        dout, dw1, db1 = conv_relu_pool_backward(dout, conv_cache)

        # 记得regularization term
        dw3 = dw3+W3 * self.reg
        dw2 = dw2+W2 * self.reg
        dw1 = dw1+W1 * self.reg
